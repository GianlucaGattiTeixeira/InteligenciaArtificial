...
signocorrecto(S,DD,MM):-
    horoscopo(S,DC,MC,DF,MF),DD>=DC,MM=MC,mes(MM,X),DD=<X).
signocorrecto(S,DD,MM):-
    horoscopo(S,DC,MC,DF,MF),(DD=<DF,MM=MF)

misigno(DD,MM):-
    horoscopo(S,DC,MC,DF,MF),(DD>=DC,MM=MC,mes(MM,X),DD=<X),write(S).
misigno(DD,MM):-
    horoscopo(S,DC,MC,DF,MF),(DD=<DF,MM=MF),write(S).